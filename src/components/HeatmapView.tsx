...
export default function HeatmapView({ markets }: { markets: Market[] }) {
  ...
  const filtered = summary
    ...
    .sort((a: LiqSymbolData, b: LiqSymbolData) => b.total - a.total);
  ...
  return (
    <div className="bg-surface border border-border1 rounded-xl overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-border1 bg-surface2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <input value={search} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)} placeholder="Search..."
            className="bg-surface border border-border1 rounded-lg px-2.5 py-0.5 text-[10px] text-text1 outline-none focus:border-accent placeholder-text3 w-20" />

          {/* Tab toggle */}
          <div className="flex items-center gap-0.5 bg-surface rounded-lg p-0.5 border border-border1">
            {(['table','feed'] as const).map(t => (
              <button key={t} onClick={() => setTab(t as typeof tab)}
                className={`px-2 py-0.5 rounded-md text-[10px] transition-all ${tab === t ? 'bg-surface2 text-text1' : 'text-text3 hover:text-text2'}`}>
                {t === 'table' ? '⊞' : '⚡'}
              </button>
            ))}
          </div>
          ...
          <button onClick={load} disabled={loading}
            className="p-1 text-text3 hover:text-text1 transition-colors disabled:opacity-40">
      ...
      {grandTotal > 0 && (
        <div className="flex items-center gap-4 px-4 py-2 border-b border-border1 bg-surface2/30">
          <div className="flex items-center gap-3 text-[11px]">
            <div>
              <span className="text-text3 text-[9px] uppercase font-semibold mr-1.5">Total</span>
              <span className="font-bold text-text1">{fmtV(grandTotal)}</span>
            </div>
            <div>
              <span className="text-text3 text-[9px] uppercase font-semibold mr-1.5">Long</span>
              <span className="font-bold text-success">{fmtV(grandLong)}</span>
            </div>
            <div>
              <span className="text-text3 text-[9px] uppercase font-semibold mr-1.5">Short</span>
              <span className="font-bold text-danger">{fmtV(grandShort)}</span>
            </div>
          </div>
          <div className="flex flex-col gap-0.5 w-24 ml-1">
          ...
          <span className="ml-auto text-[9px] text-text3 opacity-60">click row for heatmap</span>
        </div>
      )}
      ...
      {tab === 'table' && (
        <>
          {/* Column headers */}
          <div className="grid px-4 py-2 border-b border-border1 bg-surface2/50 text-[9px] uppercase tracking-wide text-text3 font-semibold"
            style={{ gridTemplateColumns: '28px 1fr 90px 80px 80px 100px' }}>
            <span>#</span>
            <span>Symbol</span>
            <span className="text-right">Total Liq</span>
            <span className="text-right">Long</span>
            <span className="text-right">Short</span>
            <span className="text-center">L/S</span>
          </div>

          {/* Scrollable rows - max 200px like Funding Spikes */}
          <div className="overflow-auto" style={{ maxHeight: 240 }}>
            {loading && filtered.length === 0 && (
              <div className="flex items-center justify-center py-8 text-text3 text-[11px] gap-2">
                <span className="animate-spin text-accent">↻</span> Fetching...
              </div>
            )}
            {!loading && filtered.length === 0 && (
              <div className="py-8 text-center text-text3 text-[11px]">No liquidation data — try a wider range</div>
            )}
            {filtered.map((s: LiqSymbolData, idx: number) => {
              ...
              return (
                <div key={s.symbol}
                  ...
                  className="grid items-center px-4 py-2.5 border-b border-border1/40 hover:bg-surface2/60 cursor-pointer transition-colors last:border-0"
                  style={{ gridTemplateColumns: '28px 1fr 90px 80px 80px 100px' }}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[9px] font-semibold ${lp >= 50 ? 'text-success' : 'text-danger'}`}>
                      {lp >= 50 ? '▲' : '▼'}
                    </span>
                  </div>

                  <div className="text-right">
                    <span className="text-[12px] font-bold text-text1">{fmtV(s.total)}</span>
                  </div>

                  <div className="text-right">
                    <span className="text-[11px] font-semibold text-success">{fmtV(s.longLiq)}</span>
                  </div>

                  <div className="text-right">
                    <span className="text-[11px] font-semibold text-danger">{fmtV(s.shortLiq)}</span>
                  </div>

                  <div className="flex h-1.5 rounded-full overflow-hidden mx-2">
      ...
      {tab === 'feed' && (
        <>
          <div className="grid px-4 py-2 border-b border-border1 bg-surface2/50 text-[9px] uppercase tracking-wide text-text3 font-semibold"
            ...
            <span>Time</span><span>Symbol</span><span>Side</span><span className="text-right">Size</span><span className="text-right">Price</span>
          </div>
          <div className="overflow-auto" style={{ maxHeight: 240 }}>
            {recent.length === 0 && !loading && (
              <div className="py-8 text-center text-text3 text-[11px]">No recent liquidations</div>
            )}
            {recent
              ...
              .map((e: LiqEvent, i: number) => {
                ...
                return (
                  <div key={e.id ?? i}
                    ...
                    className="grid items-center px-4 py-2 border-b border-border1/30 hover:bg-surface2/50 cursor-pointer transition-colors last:border-0"
                    style={{ gridTemplateColumns: '52px 1fr 60px 80px 80px' }}>
                    <span className="font-mono text-[9px] text-text3">{fmtTime(e.ts)}</span>
                    <div className="flex items-center gap-1.5">
                      <CoinLogo symbol={pm?.symbol ?? (e.symbol+'-USD')} size={14}/>
                      <span className="text-[11px] font-bold text-text1">{e.symbol}</span>
                    </div>
                    <span className={`text-[9px] font-bold ${e.side === 'long' ? 'text-success' : 'text-danger'}`}>
                      {e.side === 'long' ? '▲ L' : '▼ S'}
                    </span>
